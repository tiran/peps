...
* MD5, salted SHA256, salted SHA512, and Blowfish are optional extensions.
  ...
  algorithm that is still secure. However it is not in glibc and therefore not
...
The module depends on functions from the `audioop`_ C extension that
convert from and to ADPCM (``adpcm2lin``, ``lin2adpcm``), a-LAW (``alaw2lin``,
``lin2alaw``), and u-LAW (``ulaw2lin``, ``lin2ulaw``) encoding. TODO
...
* Keep `aifc`_, `cgi`_, `nntplib`_, and `telnetlib`_
